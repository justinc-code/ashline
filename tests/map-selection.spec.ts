...
test("map click selects a clustered volcano and connects to its description", async ({
  page,
}, testInfo) => {
  await page.goto("/");
  ...
  const marker = page
    .locator("circle[data-volcano-id]")
    .filter({ has: page.locator("title", { hasText: /^Semeru, / }) });
  ...
  await expect(
    page.getByRole("region", { name: "Description of Semeru" }),
  ).toBeVisible();
  await expect(page.locator(".atlas-pick-list")).toContainText(
    "nearby volcanoes",
  );
  await page.getByRole("button", { name: "Zoom to nearby" }).click();
  await expect(page.getByLabel("Global map zoom")).not.toHaveText("1.0×");
  const neighbor = page
    .locator(".atlas-pick-list button[aria-pressed=false]")
    .first();
  ...
  await expect(page.locator("#geology-heading")).toHaveText(
    `About ${neighborName}`,
  );
  ...
  await page.screenshot({
    path: `/private/tmp/ashline-map-selection-${testInfo.project.name}.png`,
  });
  expect(
    await page.evaluate(
...
test("drag does not select; keyboard and filtered map selection work", async ({
  page,
}) => {
  await page.goto("/");
  const semeruId = await page
    .locator("circle[data-volcano-id]")
    .filter({ has: page.locator("title", { hasText: /^Semeru, / }) })
    .getAttribute("data-volcano-id");
  await page
    .getByRole("combobox", { name: "Search map volcanoes" })
    .fill(semeruId!);
...
test("terrain relief and contours remain inspectable", async ({
  page,
}, testInfo) => {
  const errors: string[] = [];
  ...
  await page
    .getByRole("checkbox", { name: "Ash plume", exact: true })
    .uncheck();
  await page.getByRole("checkbox", { name: /Contour lines/ }).check();
  await page
    .getByRole("combobox", { name: "Ground surface" })
    .selectOption("elevation");
  await expect(page.getByLabel("Terrain elevation colors")).toContainText(
    "Elevation",
  );
  ...
  await page
    .locator(".eruption-stage")
    .screenshot({
      path: `/private/tmp/ashline-relief-${testInfo.project.name}.png`,
    });
  expect(
    await page.evaluate(
      () => document.documentElement.scrollWidth <= innerWidth,
...
test("world landscape loads surrounding DEM and satellite imagery", async ({
  page,
}, testInfo) => {
  const errors: string[] = [];
  page.on("pageerror", (error) => errors.push(error.message));
  ...
  await expect(page.locator(".world-surface-controls")).toContainText(
    "60 km surrounding DEM landscape",
    { timeout: 20000 },
  );
  await expect(page.locator(".world-surface-controls")).toContainText(
  ...
  await page
    ...
    .uncheck();
  await page
    .getByRole("checkbox", { name: "Settled ash", exact: true })
    .uncheck();
  await page
    .locator(".eruption-stage")
    .screenshot({
      path: `/private/tmp/ashline-world-${testInfo.project.name}.png`,
    });
  await page.getByRole("button", { name: "Top view", exact: true }).click();
  await page
    .locator(".eruption-stage")
    .screenshot({
      path: `/private/tmp/ashline-world-top-${testInfo.project.name}.png`,
    });
  await page
    .getByRole("combobox", { name: "Ground surface" })
    .selectOption("elevation");
...
test("world remains usable when imagery and surrounding terrain are unavailable", async ({
  page,
}) => {
  await page.route("**/terrain/10/**/*.png", (route) =>
  ...
  await expect(page.locator(".world-surface-controls")).toContainText(
    "Surrounding terrain unavailable",
  );
  await expect(page.locator(".world-surface-controls")).toContainText(
