...
test('geographic runs preserve settings, location, and mitigation choices', async ({ page }, testInfo) => {
 await page.goto('/');
 ...
 await expect(page.locator('.location-result')).toContainText('from the volcano');
 await page.getByRole('checkbox', { name: /Prepare for ash exposure/ }).check();
 await page.getByRole('button', { name: 'Run scenario', exact: true }).click();
 await expect(page.locator('.run-table tbody tr')).toHaveCount(1);
 const original = await page.locator('.run-table tbody tr').first().textContent();
 await page.getByRole('slider', { name: 'Wind speed', exact: true }).fill('100');
 await page.getByRole('button', { name: 'Run scenario', exact: true }).click();
 await expect(page.locator('.run-table tbody tr')).toHaveCount(2);
 await expect(page.locator('.run-table tbody tr').first()).toHaveText(original!);
 await expect(page.locator('.run-table tbody tr').last()).toContainText('100 km/h');
 await expect(page.locator('.run-table tbody tr').last()).toContainText('Prepare for ash exposure');
 ...
 await page.screenshot({ path: `/private/tmp/ashline-runs-${testInfo.project.name}.png`, fullPage: true });
 expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
...
test('interactive 3D eruption playback, camera, layers, and scenario inputs', async ({ page }, testInfo) => {
 const errors: string[] = []; page.on('pageerror', error => errors.push(error.message));
 await page.goto('/');
 await page.getByRole('button', { name: 'Scenario simulator', exact: true }).click();
 await expect(page.getByRole('heading', { name: 'Eruption in 3D', exact: true })).toBeVisible();
 ...
 await page.getByRole('button', { name: 'Play eruption', exact: true }).click();
 await expect.poll(async () => Number(await page.getByRole('slider', { name: 'Eruption timeline' }).inputValue())).toBeGreaterThan(25);
 await page.getByRole('button', { name: 'Pause eruption', exact: true }).click();
 await page.getByRole('slider', { name: 'Eruption timeline' }).fill('60');
 await expect(page.locator('.eruption-timeline output')).toHaveText('10.8 / 18 h');
 ...
 await canvas.focus(); await canvas.press('ArrowLeft'); await canvas.press('Home');
 await page.getByRole('checkbox', { name: 'Hazard outlines', exact: true }).check();
 await expect(page.locator('.eruption-layers')).toContainText('Ochre: ash');
 await page.getByRole('slider', { name: '3D wind speed', exact: true }).fill('60');
 await expect(page.getByRole('slider', { name: 'Wind speed', exact: true })).toHaveValue('60');
 await page.getByRole('slider', { name: /Wind toward/ }).fill('270');
 await expect(page.locator('.scene-status')).toContainText('wind 60 km/h toward 270°');
 await page.getByRole('slider', { name: 'Eruption timeline' }).fill('100');
 await page.getByRole('button', { name: 'Replay eruption', exact: true }).click();
 await expect.poll(async () => Number(await page.getByRole('slider', { name: 'Eruption timeline' }).inputValue())).toBeGreaterThan(1);
 expect(Number(await page.getByRole('slider', { name: 'Eruption timeline' }).inputValue())).toBeLessThan(20);
 await page.getByRole('button', { name: 'Pause eruption', exact: true }).click();
 await page.getByRole('slider', { name: 'Eruption timeline' }).fill('45');
 await page.getByRole('checkbox', { name: 'Hazard outlines', exact: true }).uncheck();
 await page.locator('.eruption-view').scrollIntoViewIfNeeded();
 await page.screenshot({ path: `/private/tmp/ashline-3d-${testInfo.project.name}.png`, fullPage: true });
 await page.locator('.eruption-view').screenshot({ path: `/private/tmp/ashline-3d-panel-${testInfo.project.name}.png` });
 expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
 expect(errors).toEqual([]);
 await page.getByRole('button', { name: 'Restart', exact: true }).click();
 await expect(page.getByRole('slider', { name: 'Eruption timeline' })).toHaveValue('0');
 await expect(page.locator('.scene-status')).toContainText('Before eruption');
});

test('3D context loss recovers without resetting scenario settings', async ({ page }) => {
 await page.goto('/');
 await page.getByRole('button', { name: 'Scenario simulator', exact: true }).click();
 await expect(page.getByRole('button', { name: 'Play eruption', exact: true })).toBeEnabled({ timeout: 15000 });
 await page.getByRole('slider', { name: 'Wind speed', exact: true }).fill('80');
 await page.locator('.eruption-stage canvas').dispatchEvent('webglcontextlost');
 await expect(page.getByRole('heading', { name: '3D view unavailable' })).toBeVisible();
 await expect(page.getByRole('button', { name: 'Play eruption', exact: true })).toBeDisabled();
 await page.getByRole('button', { name: 'Reload 3D view', exact: true }).click();
 await expect(page.getByRole('button', { name: 'Play eruption', exact: true })).toBeEnabled({ timeout: 15000 });
 await expect(page.getByRole('slider', { name: 'Wind speed', exact: true })).toHaveValue('80');
});

test('geographic DEM loads offline with source, true scale, and elevation inspection', async ({ page }, testInfo) => {
 await page.route('https://elevation-tiles-prod.s3.amazonaws.com/**', route => route.abort());
 await page.goto('/');
 await page.getByRole('button', { name: 'Scenario simulator', exact: true }).click();
 await expect(page.getByRole('button', { name: 'Play eruption', exact: true })).toBeEnabled({ timeout: 15000 });
 await expect(page.locator('.terrain-summary')).toContainText('20 × 20 km');
 await expect(page.locator('.terrain-summary')).toContainText('mesh spacing 78 m');
 await expect(page.getByRole('combobox', { name: 'Vertical scale' })).toHaveValue('1');
 await expect(page.locator('.terrain-inspection')).toContainText('-7.54000°, 110.44600°');
 await expect(page.getByRole('checkbox', { name: 'Hot flows unavailable on DEM' })).toBeDisabled();
 await page.getByText('Elevation data sources and limits', { exact: true }).click();
 await expect(page.locator('.terrain-provenance:not(.ash-assumptions)')).toContainText('srtm/S08E110.tif');
 await page.getByRole('combobox', { name: 'Vertical scale' }).selectOption('2');
 await expect(page.locator('.eruption-limits')).toContainText('vertical exaggeration is 2×');
 const canvas = page.locator('.eruption-stage canvas');
 const box = await canvas.boundingBox();
 await canvas.click({ position: { x: box!.width * .55, y: box!.height * .65 } });
 await expect(page.locator('.terrain-inspection')).toContainText('sampled slope');
 await page.getByRole('combobox', { name: 'Vertical scale' }).selectOption('1');
 await page.getByRole('slider', { name: 'Eruption timeline' }).fill('0');
 await page.locator('.eruption-view').scrollIntoViewIfNeeded();
 await page.locator('.eruption-view').screenshot({ path: `/private/tmp/ashline-terrain-${testInfo.project.name}.png` });
 expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
});

test('missing DEM stays explicit and allows retry or an opted-in procedural preview', async ({ page }) => {
 await page.route('**/terrain/12/**/*.png', route => route.fulfill({ status: 503, body: 'Unavailable' }));
 await page.goto('/');
 await page.getByRole('button', { name: 'Scenario simulator', exact: true }).click();
 await expect(page.getByRole('heading', { name: 'Geographic terrain unavailable' })).toBeVisible();
 await expect(page.getByRole('button', { name: 'Play eruption', exact: true })).toBeDisabled();
 ...
 await expect(page.locator('.terrain-controls')).toContainText('Procedural preview');
...
test('ash settles on the DEM, persists, and scrubs reproducibly', async ({ page }, testInfo) => {
 await page.goto('/');
 await page.getByRole('button', { name: 'Scenario simulator', exact: true }).click();
 await expect(page.getByRole('button', { name: 'Play eruption', exact: true })).toBeEnabled({ timeout: 15000 });
 await page.getByRole('slider', { name: '3D wind speed', exact: true }).fill('0');
 await page.getByRole('slider', { name: 'Ash release height', exact: true }).fill('0.5');
 await page.getByRole('combobox', { name: 'Post-eruption settling duration', exact: true }).selectOption('24');
 await page.getByRole('slider', { name: 'Eruption timeline', exact: true }).fill('100');
 await expect.poll(async () => Number(await page.getByTestId('ash-deposited').textContent())).toBe(1000);
 await expect(page.getByTestId('ash-airborne')).toHaveText('0');
 await expect(page.getByTestId('ash-outside')).toHaveText('0');
 await page.getByRole('slider', { name: 'Eruption timeline', exact: true }).fill('0');
 await expect(page.getByTestId('ash-deposited')).toHaveText('0');
 await page.getByRole('slider', { name: 'Eruption timeline', exact: true }).fill('100');
 await expect(page.getByTestId('ash-deposited')).toHaveText('1000');
 await page.getByRole('slider', { name: '3D wind speed', exact: true }).fill('25');
 await page.getByRole('slider', { name: 'Ash release height', exact: true }).fill('1.5');
 await page.getByRole('combobox', { name: 'Post-eruption settling duration', exact: true }).selectOption('6');
 await page.getByRole('slider', { name: 'Eruption timeline', exact: true }).fill('75');
 await expect(page.locator('.scene-status')).toContainText('Post-eruption settling');
 await expect.poll(async () => Number(await page.getByTestId('ash-outside').textContent())).toBeGreaterThan(0);
 await page.locator('.eruption-view').scrollIntoViewIfNeeded();
 await page.locator('.eruption-view').screenshot({ path: `/private/tmp/ashline-settling-${testInfo.project.name}.png` });
 expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
});

test('aftermath map layers, lava paths, coordinate inspection, and navigation', async ({ page }, testInfo) => {
 await page.goto('/');
 await page.getByRole('button', { name: 'Response plan', exact: true }).click();
 const panel = page.locator('.response-map');
 const map = panel.getByRole('group', { name: 'Interactive hazard map for Merapi', exact: true });
 await expect(panel.locator('[data-layer]')).toHaveCount(5);
 await panel.getByRole('checkbox', { name: 'Toxic gases', exact: true }).uncheck();
 await expect(panel.locator('[data-layer="gas"]')).toHaveCount(0);
 await panel.getByRole('checkbox', { name: 'Toxic gases', exact: true }).check();
 await panel.getByLabel('Gas screening radius').selectOption('10');
 await panel.getByRole('button', { name: 'Inspect main path end', exact: true }).click();
 await expect(panel.locator('.map-inspection')).toContainText('bearing 180°');
 await expect(panel.locator('.map-inspection')).toContainText('Lava exposure not assessed');
 await expect(panel.locator('[data-lava-path]')).toHaveCount(3);
 const originalPath = await panel.locator('[data-lava-path="Main path"]').getAttribute('d');
 await panel.getByLabel('Assumed lava heading').selectOption('90');
 await panel.getByLabel('Assumed lava reach').selectOption('10');
 await expect(panel.locator('[data-lava-path="Main path"]')).not.toHaveAttribute('d', originalPath!);
 await panel.getByRole('checkbox', { name: 'Lava flow paths', exact: true }).uncheck();
 await expect(panel.locator('[data-lava-path]')).toHaveCount(0);
 await panel.getByRole('checkbox', { name: 'Lava flow paths', exact: true }).check();
 await panel.getByRole('button', { name: 'Zoom into hazard map', exact: true }).click();
 ...
 await panel.getByRole('button', { name: 'Reset hazard map', exact: true }).click();
 const bounds = await map.boundingBox();
 await map.click({ position: { x: bounds!.width / 2, y: bounds!.height / 2 } });
 await expect(panel.locator('.map-inspection')).toContainText('0.0 km from vent');
 const before = await panel.locator('.map-inspection').textContent();
 await page.mouse.move(bounds!.x + bounds!.width / 2, bounds!.y + bounds!.height / 2);
 await page.mouse.down(); await page.mouse.move(bounds!.x + bounds!.width / 2 + 40, bounds!.y + bounds!.height / 2 + 30, { steps: 5 }); await page.mouse.up();
 await expect(panel.locator('.map-inspection')).toHaveText(before!);
 await panel.getByRole('button', { name: 'Reset hazard map', exact: true }).click();
 await expect(page.getByRole('checkbox', { name: /Check toxic gases before re-entry/ })).toBeVisible();
 await panel.screenshot({ path: `/private/tmp/ashline-aftermath-${testInfo.project.name}.png` });
 expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
 await page.getByRole('button', { name: 'Edit scenario', exact: true }).click();
 const lab = page.locator('.scenario-lab');
 await lab.getByRole('button', { name: 'Inspect main path end', exact: true }).click();
 ...
 await page.getByRole('slider', { name: 'Explosivity · VEI', exact: true }).fill('0');
 await expect(lab.locator('[data-lava-path]')).toHaveCount(3);
});

test('global reference map supports search, focus, pan, pinch, and selection', async ({ page }, testInfo) => {
 const errors: string[] = []; page.on('pageerror', e => errors.push(e.message));
 await page.goto('/');
 ...
 await expect(map.locator('g').first()).not.toHaveAttribute('transform', camera!);
 await map.press('Home');
 await expect(page.getByLabel('Global map zoom')).toHaveText('1.0×');
 await page.getByRole('textbox', { name: 'Search map volcanoes', exact: true }).fill('Merapi');
 await expect(page.getByRole('textbox', { name: 'Search volcanoes', exact: true })).toHaveValue('Merapi');
 await page.getByRole('button', { name: 'Fit matches', exact: true }).click();
 expect(Number((await page.getByLabel('Global map zoom').innerText()).replace('×', ''))).toBeGreaterThan(1);
 await page.getByRole('button', { name: 'Focus selected', exact: true }).click();
 const box = await map.boundingBox();
 await map.click({ position: { x: box!.width / 2, y: box!.height / 2 } });
 await expect(page.locator('.atlas-pick-list')).toContainText('Merapi');
 await page.locator('.atlas-pick-list').getByRole('button', { name: /Merapi Indonesia/ }).click();
 ...
 await cdp.send('Input.dispatchTouchEvent', { type: 'touchStart', touchPoints: [firstTouch, { x: touchBox.x + 180, y: touchBox.y + 100, id: 2 }] });
 await cdp.send('Input.dispatchTouchEvent', { type: 'touchMove', touchPoints: [firstTouch, { x: touchBox.x + 260, y: touchBox.y + 100, id: 2 }] });
 await cdp.send('Input.dispatchTouchEvent', { type: 'touchEnd', touchPoints: [] });
 await cdp.detach();
 await expect(page.getByLabel('Global map zoom')).toHaveText('1.8×');
 await page.getByRole('textbox', { name: 'Search map volcanoes', exact: true }).fill('no-such-volcano-xyz');
 ...
 await page.getByRole('button', { name: 'Clear map search', exact: true }).click();
 ...
 await expect(page.getByRole('button', { name: 'Build a scenario', exact: true })).toBeVisible();
 await page.getByRole('button', { name: 'Collapse map', exact: true }).click();
 await page.getByRole('button', { name: 'Reset map zoom', exact: true }).click();
 await page.locator('.map-panel').screenshot({ path: `/private/tmp/ashline-global-${testInfo.project.name}.png` });
 expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
 expect(errors).toEqual([]);
});
